...
class Node:
    ...
    def __repr__(self):
        return f"Position: {self.position} g(n): {self.g} h(n): {self.h} f(n): {self.f}"
...
def A_star(maze):
    ...
    start = Node(None, find_start(maze))  # gives 0
    currPosition = start
    goal = Node(None, find_endpoint(maze))  # gives 0
    # g, h, f values are set to 0 (normally inf)
    start.g = start.h = start.f = 0
    goal.g = goal.h = goal.f = 0
    # Add start node to fringe
    open.append(start)
    # # while len(stack) is not empty:
    while len(open) > 0:

        currPosition = open[0]
        index = 0
        for i, node in enumerate(open):
            if node.f < currPosition.f:
                currPosition = node
                index = i
        open.pop(index)
        closed.append(currPosition)
        ...
        for newPosition in [
            ...
            nodePosition = (
                currPosition.position[0] + newPosition[0],
                currPosition.position[1] + newPosition[1],
            )
            print(currPosition)
            if (
                nodePosition[0] > (len(maze) - 1)
                or nodePosition[0] < 0
                or nodePosition[1] > (len(maze[len(maze) - 1]) - 1)
                or nodePosition[1] < 0
            ):
                continue
            ...
            updatedNode = Node(currPosition, nodePosition)
            currPosition = "o"

            childArr.append(updatedNode)
        for c in childArr:
            if len([closed_c for closed_c in goal if closed_c == c]) > 0:
                continue

        c.g = currPosition.g + 1
        c.h = ((c.position[0] - goal.position[0]) ** 2) + ((c.position[1]) ** 2)
        c.f = c.g + c.h
        ...
        #             if c.position == openNode.position and c.g > openNode.g
        #         ]
        #     )
        #     > 0
        # ):
